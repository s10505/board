(function () {
    ...
    let KOSHATboard = class {
        ...
        TboardAuth = class { //화면 권한 객체
            ...
            isShowBtnWrite() {
                if (this.isBasicWrite() && this.tboardInstance.bbsUser.isLogin()) {
                    return true;
                }
                return false;
            }
        ...
        TboardArtcl = class { //화면 컴포넌트
            ...
            writeType1(_viewType) {
                ...
                //게시판유형(갤러리)
                if (this.tboardInstance.bbsAuth.isGalleryType() || this.tboardInstance.bbsAuth.isThumbListType()) {
            ...
            CommentType1Component = class {
                ...
                appendComment(cmntList) {
                    ...
                    cmntList.forEach(data => {
                        if (this.isDelCmntDisp === false) { //삭제 데이터 표시 여부
                            if (data.delYn === "Y") {
                                return;
                            }
                        }
                ...
                CommentItem = class {
                    ...
                    setWriteData(_isEdit) {
                        ...
                        this.commentEl.write.cmntCn.textContent = "";
                        if (this.editMode === "U") { //수정모드
                            this.commentEl.write.cmntCn.textContent = this.commentData.cmntCn;
                        }
                        //비밀글
                        let isChecked = false;
                        if (this.commentData.rlsYn === "N") {
            ...
            ListType2Component = class {
                ...
                setThumb(_artclNo, _thumbInfo, _pstInfo) {
                    ...
                    if (Array.isArray(_thumbInfo) && _thumbInfo.length > 0) {
                        imageBase64 = _thumbInfo[0].imageBase64;
                    }
            ...
            WriteComponent = class {
                ...
                init() {
                    if (this.optType === "pstSeCd") {
                        ...
                        if (this.labelNode) {
                            if (this.artclInfo.title) {
                                this.labelNode.textContent = this.artclInfo.title || "";
                            }
                            else {
                            ...
                            if (initValue) {
                                ...
                                template.content.childNodes.forEach((tempEl) => {
                                    if (isFirstLine) {
                                        ...
                                        quillLength = this.editor.getLength();
                                    }

                                    if (tempEl.nodeType === 1) {
                                        let elData = tempEl.outerHTML;
                                        elData = elData.replace(/<br\s*\/?>/g, "\n");
                                        this.editor.clipboard.dangerouslyPasteHTML(quillLength, elData);
                                    }
                                    else if (tempEl.nodeType === 3) {
                                        let elData = tempEl.textContent;
                                        elData = elData.replace(/\r?\n/g, "<br>");
                                        this.editor.clipboard.dangerouslyPasteHTML(quillLength, elData);
                                    }
                                });
                            }
                ...
                getData() {
                    ...
                    if (this.subType === "notice") {
                        ...
                        if (this.editor) {
                            ...
                            let isFlag = tboardUtil.isQuillEditorText(content);
                            if (isFlag === false) {
                                content = "";
                            }
                            else {
            ...
            async savePst(_data) {
                ...
                try {
                    ...
                    Object.keys(writeCmps).forEach(_key => {
                        ...
                        try {
                            if (isRequired.toUpperCase() === "Y") {
                                //첨부파일 필수 체크 추가
                                if (component.type.indexOf("atch") > -1) {
                                    let fileCnt = 0;
                                    if (Array.isArray(objData.curFiles)) {
                                        fileCnt = objData.curFiles.length;
                                    }
                                    fileCnt = fileCnt + Object.keys(objData.data).length;
                                    if (fileCnt < 1) {
                                        objRslt.isErr = true;
                                    }
                                }
                        ...
                        try {
                            ...
                            if (isNaN(length)) {
                                ...
                                checkByte = Number(length);
                            }

                            //입력 값 길이 체크
                            if (component.type.indexOf("input") > -1 && checkByte > 0) {
                                if (typeof objData.data === "string") {
                                    wordByte = tboardUtil.getByteLength(objData.data, 3);
                                }
                    ...
                    if (!writeCmps["D010100001"]) {
                        objPstWriteGrid.pstNm = "Empty";
                    }
                    //내용 항목 자체가 없는 경우
                    if (!writeCmps["D010200001"]) {
                        objPstWriteGrid.pstCn = "Empty";
                    }
        ...
        TboardEvent = class {
            ...
            addSearchDefaultEvent() {
                ...
                if (writeBtn) {
                    ...
                    if (this.tboardInstance.bbsAuth.isBasicManage() && this.tboardInstance.bbsAuth.isEditLock()) {
                        writeBtn.remove();
                        return;
                    }
            ...
            async searchData() {
                ...
                try {
                    if (Object.keys(this.tboardInstance.result.search).length < 1) {
                        this.tboardInstance.result.search = this.tboardInstance.result.mergeSearch;
                    }
                    //게시물 조회 정보(백업)
                    ...
                    if (this.tboardInstance.result.mergeSearch.baseList.length > 0) {
                        tboardUtil.arraySort(this.tboardInstance.result.mergeSearch.baseList, "rnum", true, "number");
                    }
                }
                catch (err) {
            ...
            async detailViewDefault(_objPst) {
                ...
                //상세로 바로 접근 시 페이지번호 받아서 처리하는 경우(이전,다음글 처리 대응코드)
                if (_objPst.pageNo !== undefined && _objPst.pageNo !== null) {
                    let pageNo = _objPst.pageNo;
                    if (isNaN(pageNo)) {
                        pageNo = 1;
                    }
                    this.tboardInstance.bbsPaging.setCurrentPageNo(pageNo);
                    let searchData = await this.searchData();
                    ...
                    this.tboardInstance.bbsPaging.setTotalPageNo();
                }

                //상세
                detailPromiseFn = this.tboardInstance.bbsFn.searchDetail(_objPst);
                promiseFnArr.push(detailPromiseFn);

                //답글
                if (this.tboardInstance.bbsAuth.isReplyRead()) {
                    replyPromiseFn = this.tboardInstance.bbsFn.searchReplyDetail(_objPst);
                    promiseFnArr.push(replyPromiseFn);
                }
                //댓글
                if (this.tboardInstance.bbsAuth.isCommentRead()) {
                    cmntPromiseFn = this.tboardInstance.bbsFn.searchCmntList(_objPst);
            ...
            async handleEvent(_args) {
                ...
                if (_args.key === "basicAccess.tx") {
                    ...
                    let result = await this.tboardInstance.bbsFn.savePst(data);
                    return result;
                }
        ...
        TboardPaging = class {
            ...
            setTotalPageNo() {
                //this.totalNormalCnt = _totalNormalCnt;
        ...
        Tboard = class {
            ...
            async build(_args) {
                ...
                //화면표시
                if ((_args.initView || "").toLowerCase() === "write") {
                    //등록화면
                    this.initView = "write";
                    await this.showViewWrite();
                }
                else if ((_args.initView || "").toLowerCase() === "detail" || _args.pstNo) {
                    if (this.bbsAuth.isFaqType()) {
                        let isPagingInit = false;
                        let pageNo = _args.pageNo || 1;
                        if (isNaN(pageNo)) {
                            pageNo = 1;
                        }

                        this.bbsPaging.setCurrentPageNo(pageNo);
                        //FAQ(상세없음)로 게시물번호가 들어오면, 목록조회 후 클릭이벤트 발생
                        this.initView = "list";
                        await this.showViewList(_args.pstNo, isPagingInit);

                        if (_args.pstNo) {
                            let faqTitle = this.view.search.querySelector(`[data-tboard-pst-no="${_args.pstNo}"]`);
                            if (faqTitle) {
                                faqTitle.click();
                            }
                        }
                        ...
                        //상세화면
                        this.initView = "detail";
                        await this.showViewDetail(_args);
                    }
                }
                else {
                    //조회화면
                    this.initView = "list";
                    await this.showViewList();
                }

                return this.rtnData;
            }

            async showViewList(_pstNo, _isPagingInit) {
                let isPagingInit = true;
                if (_isPagingInit === false) {
                    isPagingInit = _isPagingInit;
                }
                //기본화면 세팅(화면 타입별 조회 화면 생성)
                this.bbsEvent.searchView({ isSearch: false, isPagingInit: isPagingInit });

                await this.bbsEvent.search();
            }

            async showViewDetail(_args) {
                let inputData = {};
                inputData.bbsId = _args.bbsId;
                inputData.pstNo = _args.pstNo;
                inputData.pageNo = _args.pageNo || 1;
                            ...
                            if (value) {
                                ...
                                template.content.childNodes.forEach((tempEl) => {
                                    if (isFirstLine) {
                                        ...
                                        quillLength = editor.getLength();
                                    }

                                    if (tempEl.nodeType === 1) {
                                        let elData = tempEl.outerHTML;
                                        elData = elData.replace(/<br\s*\/?>/g, "\n");
                                        editor.clipboard.dangerouslyPasteHTML(quillLength, elData);
                                    }
                                    else if (tempEl.nodeType === 3) {
                                        let elData = tempEl.textContent;
                                        elData = elData.replace(/\r?\n/g, "<br>");
                        ...
                        if (this.bbsArtcl.cmp.writeReply.upPst[artclNo].type === "inputType1") {
                            ...
                            template.content.childNodes.forEach((tempEl) => {
                                if (isFirstLine) {
                                    ...
                                    quillLength = this.bbsArtcl.cmp.writeReply.upPst[artclNo].editor.getLength();
                                }

                                if (tempEl.nodeType === 1) {
                                    let elData = tempEl.outerHTML;
                                    elData = elData.replace(/<br\s*\/?>/g, "\n");
                                    this.bbsArtcl.cmp.writeReply.upPst[artclNo].editor.clipboard.dangerouslyPasteHTML(quillLength, elData);
                                }
                                else if (tempEl.nodeType === 3) {
                                    let elData = tempEl.textContent;
                                    elData = elData.replace(/\r?\n/g, "<br>");
            ...
            async getPrevAndNextPstNo(_pstNo) {
                ...
                try {
                    let filteredData = this.result.search.baseList.filter(data => data.pstNo === _pstNo);
                    if (filteredData.length > 0) {
                        //페이지번호 세팅
                        this.bbsPaging.calcCurrentPage(filteredData[0].rnum);
                    }
                }
                catch (err) {
                    console.log(err);
        ...
        TboardMsg = class {
            ...
            magIdMap(_msgCode) {
                this.msgInfo = {
                    ...
                    "msg_103": "게시글을 삭제 하시겠습니까?",
                    "msg_104": "게시글을 등록 하시겠습니까?",
                    "msg_105": "게시글을 수정 하시겠습니까?",
                    "msg_106": "게시글(답글)을 삭제 하시겠습니까?",
                    "msg_107": "게시글(답글)을 등록 하시겠습니까?",
                    "msg_108": "게시글(답글)을 수정 하시겠습니까?",
                    "msg_109": "댓글을 삭제 하시겠습니까?",
                    "msg_110": "댓글을 등록 하시겠습니까?",
                    "msg_111": "댓글을 수정 하시겠습니까?",
                    ...
                    "msg_802": "게시판 요청 값에 오류가 있습니다.[시스템코드]\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_803": "게시판 요청 값에 오류가 있습니다.[채널코드]\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_804": "게시판 요청 값에 오류가 있습니다.[게시판아이디]\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_805": "게시판 요청 값에 오류가 있습니다.[서비스아이디]\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_806": "게시판 정보를 조회할 수 없습니다. 확인 후 다시 시도하시기 바랍니다.",
                    "msg_807": "등록 가능한 파일 수를 초과하였습니다.",
                    "msg_808": "허용되지 않은 파일이 있습니다.",
                    "msg_809": "파일크기를 확인하세요.",
                    "msg_810": "게시판 사용항목이 설정 되지 않았습니다. 관리자에게 문의하시기 바랍니다.",
                    "msg_811": "게시판 권한이 없습니다. 관리자에게 문의하시기 바랍니다.",
                    "msg_812": "첨부파일 업로드 권한이 없습니다.",
                    "msg_813": "로그인 후 게시글을 등록(수정) 할 수 있습니다.",

                    "msg_850": "댓글 처리를 위한 입력값이 없습니다.\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_851": "입력 값 오류가 있습니다.[게시판아이디]\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_852": "입력 값 오류가 있습니다.[게시물번호]\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_853": "댓글내용은 필수 입력 입니다.\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_854": "로그인 후 댓글을 등록/수정 할 수 있습니다.\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_855": "작성자가 아니면 댓글을 수정할 수 없습니다.",
                    "msg_856": "작성자가 아니면 댓글을 삭제할 수 없습니다.",
                    "msg_857": "수정 할 댓글이 없습니다.\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_858": "삭제된 댓글입니다.\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_859": "로그인 후 댓글을 삭제 할 수 있습니다.\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_860": "입력 값 오류가 있습니다.[댓글번호]\n확인 후 다시 시도하시기 바랍니다.",
                    "msg_861": "삭제 할 댓글이 없습니다.\n확인 후 다시 시도하시기 바랍니다.",
                    ...
                    "msg_919": "{0}의 입력 가능한 글자수가 초과 되었습니다.",
